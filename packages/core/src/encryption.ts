		...
		try {
			...
			return Err(
				createError({
					...
					params: {
						env: `window=${typeof window !== "undefined"} process=${typeof process !== "undefined"}`,
...
async function encryptChunk(publicKey: CryptoKey, chunk: Uint8Array) {
  ...
  if (cryptoResult.err === true) {
	...
	return Ok(await cryptoResult.val.subtle.encrypt(
		{
			name: 'RSA-OAEP'
		},
		publicKey,
...
async function decryptChunk(privateKey: CryptoKey, chunk: ArrayBuffer) {
  ...
  if (cryptoResult.err === true) {
	...
	return Ok(await cryptoResult.val.subtle.decrypt(
		{
			name: 'RSA-OAEP'
		},
		privateKey,
...
export async function decrypt(privateKeyString: string, ciphertext: string) {
	...
	for (const base64Chunk of base64Chunks) {
		...
		const decryptedChunkResult = await decryptChunk(privateKey, chunkBufferResult.val)
...
export async function encryptLogObject<T>(log: Log<T>, encryptKeys: (keyof Log<T>)[], publicKey: string) {
  ...
  for (let i = 0; i < encryptKeys.length; i++) {
    ...
    const encryptResult = await encrypt(publicKey, JSON.stringify(valueToEncrypt))
