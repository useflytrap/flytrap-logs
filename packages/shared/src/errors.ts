...
export const createError = createHumanLogs({
	events: {
    fix_type_event: '',
		config_invalid: 'The configuration passed to `createFlytrapLogger` is invalid',
    ...
    api_unreachable: 'because we could not reach the Flytrap API.',
		...
		encrypting_required_keys: {
			template: 'because you passed in the key(s) {keys} to `encryptKeys`, which is a required key, meaning you cannot encrypt it.',
			params: {
				keys: '',
			}
		},
		encryption_enabled_without_pubkey: 'because you have enabled encryption, but have not provided a public key.',
		...
		crypto_instance_not_found: {
			template: 'because we could not find the crypto instance for the "{env}" environment. {error}',
			params: {
				env: '',
				error: ''
			}
    ...
    // Generic solutions
		open_issue: {
			template: "If you think this error shouldn't be happening, open an issue on GitHub and provide the code that caused this error.",
			params: {},
			actions: [
				{
					text: 'Open an issue',
